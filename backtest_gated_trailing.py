...
exit — all untouched) with an ADDED gated trailing stop: dormant until a trade's
price move reaches `activation_pct` in its favor, then continuously trails
`trail_pct` behind the running peak (never loosens). The channel exit stays
active in parallel throughout — this doesn't replace it, unlike
backtest_trailing.py's experiment.
"""
...
def run_gated_trailing_backtest(
    df: pd.DataFrame,
    ...
    activation_pct: Optional[float] = 0.10,  # None = disabled (reproduces production)
    ...
    for ts, row in data.iterrows():
        ...
        # --- Gated trailing stop: dormant until activation_pct reached, then ratchets ---
        if pos_side is not None and activation_pct is not None:
            if pos_side == "Buy":
                pos_extreme = max(pos_extreme, high_p)
                if not pos_trail_armed and high_p >= pos_entry_price * (1 + activation_pct):
                    pos_trail_armed = True
                    trail_arm_count += 1
                if pos_trail_armed:
                    candidate = pos_extreme * (1 - trail_pct)
                    pos_stop_price = max(pos_stop_price, candidate)
            else:
                pos_extreme = min(pos_extreme, low_p)
                if not pos_trail_armed and low_p <= pos_entry_price * (1 - activation_pct):
                    pos_trail_armed = True
                    trail_arm_count += 1
                if pos_trail_armed:
                    candidate = pos_extreme * (1 + trail_pct)
        ...
        if pos_side == "Buy":
            if low_p <= pos_stop_price:
                ...
                exit_reason = "gated_trailing_stop" if pos_trail_armed else "stop_loss"
        elif pos_side == "Sell":
        ...
        if pos_side is not None and stopped_out:
            ...
            pos_extreme, pos_trail_armed, pos_entry_time = 0.0, False, None
    ...
    trailed = trades_df[trades_df["exit_reason"] == "gated_trailing_stop"] if not trades_df.empty else pd.DataFrame()
    ...
    return {
        ...
        "trail_arm_count": trail_arm_count, "trailed_exits": len(trailed),
    }


def main():
    parser = argparse.ArgumentParser(description="Production strategy with an added gated trailing stop")
    ...
    parser.add_argument("--trail-pct", type=float, default=0.05)
    parser.add_argument("--disable", action="store_true")
    args = parser.parse_args()

    df = load_dataset()
    res = run_gated_trailing_backtest(
        df, start_date=args.start, end_date=args.end, capital_policy=args.policy,
        activation_pct=None if args.disable else args.activation_pct, trail_pct=args.trail_pct,
    )
    print_summary(res)
    print(f"Trail armed: {res['trail_arm_count']}  Exits via trailing stop: {res['trailed_exits']}")
